...
def main():
    ...
    dados = list()
    empresas = [f"{empresa}: {usuario.empresas[empresa]['Nome']}" for empresa in usuario.empresas]
    ...
    coluna_carregando = [
        [sg.Push(), sg.Image(ring_gray_segments_big, key="animacao_dados_lentes"), sg.Push()],
        [sg.Text("Aguarde enquanto a grade é baixada.")]
    ]

    layout_lentes = [
        [sg.Push(), sg.Column(coluna_lentes_grau), sg.Column(coluna_lentes_resultados), sg.Push()],
        [sg.Column(coluna_carregando, key="coluna_carregando")]
    ]
    ...
    coluna_carregando_e = window_principal['coluna_carregando'].widget.master
    botao_download.place(in_=window_principal['tabela'].widget, anchor="center", relx=0.5, rely=0.5, bordermode=sg.tk.OUTSIDE)
    coluna_carregando_e.place(in_=window_principal['tab_lentes'].widget, anchor="center", relx=0.5, rely=0.5, bordermode=sg.tk.OUTSIDE)
    ...
    while True:
        ...
        if event == sg.WINDOW_CLOSED:
            ...
            threading.Thread(target=usuario.extrair_dados_pedidos, daemon=True).start()
        ...
        elif event == "carregou_grade":
            window_principal['buscar'].update(disabled=False)
            coluna_carregando_e.place(in_=window_principal['tab_lentes'].widget, anchor="center", relx=2, rely=2, bordermode=sg.tk.OUTSIDE)

        [window_principal[key].update_animation(ring_gray_segments_big, time_between_frames=100) for key in ["animacao_dados", "animacao_dados_lentes"]]
...
class Usuario:
    def __init__(self, dados_login):
        ...
        self.empresas = self.verificar_empresas()
        self.codigo_empresa = [empresa for empresa in self.empresas][0]
        threading.Thread(target=self.pegar_grades, name="grades", daemon=True).start()
        ...
        try:
            ...
            if len(requisicao) > 1:
                for req in requisicao:
                    ...
                    window_principal.write_event_value("carregando", [self.pedidos[req['PEDIDO']], req['DESCRICAO'].strip("Lente Haytek Visão Simples Acabada"), req['NOME'], req['OSCLI'], req['DIR_ESFER'], req['DIR_CIL'], req['ESQ_ESFER'], req['ESQ_CIL'], req['DIR_ADD'], req['ESQ_ADD'], req['PEDIDO'], locale.currency(req['VALOR'], grouping=True)])
            else:
    ...
    def extrair_dados_pedidos(self):
        ...
        if self.pedidos is not None:
            ...
            window_principal.write_event_value("carregou", '')

        else:
            window_principal.write_event_value("falhou", '')
    ...
    def verificar_dioptria(self, dioptria):
        ...
        for cod_lente, faixa in self.grades.items():
            for olho, grau in dioptria.items():
                if grau['esf'] in faixa:
                    cil = faixa[grau['esf']].split("/")
                    if float(cil[0]) <= grau['cil'] <= float(cil[1]):
                        if not cod_lente in disponibilidade:
                            disponibilidade[cod_lente] = {}
                        disponibilidade[cod_lente][olho] = True
    ...
    def pegar_grades(self):
        ...
        window_principal.write_event_value("carregou_grade", "")
        
    def grades_auxiliar(self, lente):
        lista_cilindros = list()
        self.grades[lente] = {}
        for grau in self.requisicoes_get(f"https://api.haytek.com.br/v1.1/lens/{lente}/opccod").json()['RESULT']:
            for cilindro in grau:
                if cilindro.startswith("N_") and grau[cilindro] != "":
                        cilindro = cilindro.strip("N_")
                        len_cil = len(cilindro)
                        cil = f"{cilindro[0:len_cil-2]}.{cilindro[len_cil-2]}{cilindro[len_cil-1]}"
                        lista_cilindros.append(float(f"-{cil}") if float(cil) != 0 else float(f"{cil}"))
            self.grades[lente][grau['ESFERICO']] = f"{min(lista_cilindros)}/{max(lista_cilindros)}"
